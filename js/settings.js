const Settings = (function () {
  ...
  Settings.setup = function (_game) {
    ...
    $closeSettingsBtn.addEventListener("pointerdown", Settings.toggleVisibility)
    $enableSoundBtn.addEventListener("pointerdown", () => { Settings.enableSound(!Settings.isSoundEnabled) })
    $reloadCartonBtn.addEventListener("pointerdown", Game.newCarton.bind(Game));
    $colorSelector.addEventListener("change", (e) => { Settings.setCartonColor(e.target.value) });
    $markerColorSelector.addEventListener("change", (e) => { Settings.setMarkerColor(e.target.value) });
  }
  ...
  Settings.toggleVisibility = function () {
    toggleOverlay()
    $settingsSection.classList.toggle("roll-down")
  }
